  ...
  useEffect(() => {
    if (currentSection === "Introduction") {
      ...
      const parentMenu = menuItems.find(
        (item) =>
          item.subItems?.includes(currentSection) ||
      ...
      if (parentMenu) {
        ...
        if (
          parentMenu.subItems &&
          !parentMenu.subItems.includes(currentSection)
        ) {
            ...
            <div key={item.title} className="space-y-2">
              <div
                className={`flex items-center justify-between cursor-pointer ${
                  isActive(item.title) ||
                  (item.subItems && item.subItems.includes(currentSection))
                    ? "text-[#F57618]"
                    : "text-[#313D4F]"
                }`}
                onClick={() => {
                ...
                <div className="pl-7 space-y-2">
                  {item.subItems.map((subItem) => (
                    <Link
                      ...
                      href="#"
                      className={`block ${
                        isActive(subItem) ? "text-[#F57618]" : "text-[#313D4F]"
                      }`}
                      onClick={(e) => {
                        e.preventDefault();
                        onNavigate(subItem);
